...
public:
    ...
    virtual void onRead(const std::string &portName, const char *buf, size_t len, const std::string &error) {
        ...
        cout << "GPIO收到数据,portName:" << portName << ",data:" << std::string(buf, len) << ",len:" << len << endl;
    }
    // 数据写入之后
...
int main(int argc, char *argv[]) {
    std::string name = "/dev/DIN3";
    std::string name1 = "/dev/DIN4";
    GPIOTest port;
    std::string error;
    if (!port.add(error, name)) {
        cout << "GPIO打开失败,name" << name << ",error:" << error << endl;
        return 0;
    }
    if (!port.add(error, name1)) {
        cout << "GPIO打开失败,name" << name1 << ",error:" << error << endl;
        // return 0;
    }
